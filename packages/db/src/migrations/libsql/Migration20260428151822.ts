...
export class Migration20260428151822 extends Migration {
  ...
  override up(): void | Promise<void> {
    this.addSql(`create table \`app_settings\` (\`id\` text not null primary key default 'default', \`sender_name\` text not null default '', \`sender_address_street\` text not null default '', \`sender_address_postal_code\` text not null default '', \`sender_address_city\` text not null default '', \`sender_phone\` text null, \`sender_fax\` text null, \`sender_email\` text null, \`sender_bank_name\` text null, \`sender_bank_iban\` text null, \`sender_bank_bic\` text null, \`logo_mode\` text not null default 'app', \`logo_alignment\` text not null default 'center', \`logo_scale_percent\` integer not null default 100, \`logo_storage_key\` text null, \`logo_mime_type\` text null, \`ai_provider\` text null, \`ai_api_key\` text null, \`ai_base_url\` text null, \`ai_model\` text null, \`climate_factors_auto_fetch\` integer null, \`update_check_enabled\` integer null, \`telemetry_enabled\` integer null, \`installation_id\` text null, \`telemetry_last_sent_at\` text null, \`last_app_version\` text null, \`created_at\` text not null default current_timestamp, \`updated_at\` text not null default current_timestamp);`);
    ...
    this.addSql(`create table \`cost_entries\` (\`id\` text not null primary key, \`building_id\` text not null, \`invoice_date\` text not null, \`invoice_number\` text null, \`vendor\` text null, \`document_path\` text null, \`notes\` text null, \`created_at\` text not null default current_timestamp, \`updated_at\` text not null default current_timestamp, constraint \`cost_entries_building_id_foreign\` foreign key (\`building_id\`) references \`buildings\` (\`id\`) on delete restrict);`);
    ...
    this.addSql(`create table \`meters\` (\`id\` text not null primary key, \`building_id\` text not null, \`unit_id\` text null, \`type\` text not null, \`role\` text not null, \`label\` text not null, \`serial_number\` text null, \`measurement_unit\` text not null, \`room\` text null, \`cost_allocation_mode\` text not null default 'cost_types', \`radiator\` text null, \`k_total\` double null, \`radiator_manufacturer\` text null, \`radiator_model\` text null, \`radiator_type\` text null, \`radiator_dimensions\` text null, \`valid_from\` text not null, \`valid_until\` text null, \`reset_day\` text null, \`is_active\` integer not null default true, \`is_remote_readable\` integer not null default false, \`created_at\` text not null default current_timestamp, \`updated_at\` text not null default current_timestamp, constraint \`meters_building_id_foreign\` foreign key (\`building_id\`) references \`buildings\` (\`id\`) on delete restrict, constraint \`meters_unit_id_foreign\` foreign key (\`unit_id\`) references \`units\` (\`id\`) on delete restrict);`);
    ...
    this.addSql(`create unique index \`operating_cost_statements_reference_unique\` on \`operating_cost_statements\` (\`sequence_year\`, \`sequence_number\`, \`revision_number\`);`);
