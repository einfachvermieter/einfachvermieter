...
export class Migration20260428151822 extends Migration {
  ...
  override up(): void | Promise<void> {
    ...
    this.addSql(`create table \`cost_types\` (\`id\` text not null primary key, \`building_id\` text not null, \`name\` text not null, \`category\` text not null default 'operating', \`default_allocation_key\` text null, \`labor_cost_category\` text null, \`co2_tracked\` integer not null default false, \`description\` text null, \`created_at\` text not null default current_timestamp, \`updated_at\` text not null default current_timestamp, constraint \`cost_types_building_id_foreign\` foreign key (\`building_id\`) references \`buildings\` (\`id\`) on delete restrict);`);
    ...
    this.addSql(`create table \`meters\` (\`id\` text not null primary key, \`building_id\` text not null, \`unit_id\` text null, \`type\` text not null, \`role\` text not null, \`label\` text not null, \`serial_number\` text null, \`measurement_unit\` text not null, \`room\` text null, \`cost_allocation_mode\` text not null default 'cost_types', \`radiator\` text null, \`k_total\` double null, \`radiator_manufacturer\` text null, \`radiator_model\` text null, \`radiator_type\` text null, \`radiator_dimensions\` text null, \`valid_from\` text not null, \`valid_until\` text null, \`is_active\` integer not null default true, \`created_at\` text not null default current_timestamp, \`updated_at\` text not null default current_timestamp, constraint \`meters_building_id_foreign\` foreign key (\`building_id\`) references \`buildings\` (\`id\`) on delete restrict, constraint \`meters_unit_id_foreign\` foreign key (\`unit_id\`) references \`units\` (\`id\`) on delete restrict);`);
